...
export default {
  ...
  created() {
    axios
      .get(this.$apiURL + "products/" + this.$route.params.id)
      .then(res => (this.OneItem = res.data));
  },
  methods: {
    deleteitem: function() {
      axios.delete(this.$apiURL + "products/" + this.$route.params.id);
      this.$router.push("/products");
    },
    postNow: function() {
      axios
        .post(this.$apiURL + "products/" + this.$route.params.id + "/update", {
          data: this.OneItem
        })
